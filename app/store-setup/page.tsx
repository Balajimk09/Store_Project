...
export default function StoreSetupPage() {
  ...
  useEffect(() => {
    if (!loading && !user) {
      router.push('/login?redirect=/store-setup');
    }
  }, [loading, user, router]);
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="mx-auto max-w-2xl px-4 py-10 sm:px-6 lg:px-8">
        <div className="mb-6 flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-semibold tracking-tight text-foreground">{store ? 'Your store' : 'Set up your store'}</h1>
            <p className="mt-0.5 text-sm text-muted-foreground">
              {store ? 'Your store details are synced to the cloud.' : 'Tell us about your store to enable cloud sync.'}
            </p>
          </div>
        </div>
