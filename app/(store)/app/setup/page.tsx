...
export default function SetupPage() {
  ...
  useEffect(() => {
    if (!loading && !user) {
      router.push('/login?redirect=/app/setup');
    }
  }, [loading, user, router]);
    ...
    <div className="min-h-screen bg-background">
      <div className="border-b border-border bg-sidebar px-6 py-4">
        <div className="mx-auto flex max-w-4xl items-center justify-between">
          <Link href="/app/dashboard" className="flex items-center gap-2.5 text-white">
          ...
          <Link href="/app/account" className="text-sm text-white/80 hover:text-white hover:underline">
            Account
          </Link>
        </div>
      </div>
