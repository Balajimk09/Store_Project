-- Add optional explicit POS business_date support for closed Verifone periods.
-- Preserves the unlocked canonical transaction upsert signature and behavior.

create or replace function public.upsert_pos_transaction_unlocked(
  p_store_id uuid,
  p_owner_id uuid,
  p_connector_id uuid,
  p_import_id uuid,
...
comment on function public.upsert_pos_transaction_unlocked(uuid, uuid, uuid, uuid, jsonb) is
  'Internal backend-only atomic idempotent upsert for one canonical normalized POS transaction. Supports optional explicit POS business_date for closed periods.';
