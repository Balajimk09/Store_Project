-- Connector-facing queue RPCs. These functions return only the data required for update_price work.

create or replace function public.pos_publish_failure_message_is_safe(p_failure_message text)
returns boolean
language sql
immutable
set search_path = pg_catalog
as $$
  select p_failure_message is null
    or (
      length(p_failure_message) between 1 and 240
      and p_failure_message !~ '[[:cntrl:]]'
      and p_failure_message !~* 'authorization|bearer|basic[[:space:]]+auth|access[_[:space:]]?token|refresh[_[:space:]]?token|token|set-cookie|cookie|password|passwd|secret|api[_[:space:]]?key|apikey|session|credential|private[_[:space:]]?key|service(_|[[:space:]]|-)?role|supabase[_[:space:]]?key|stack[[:space:]]*trace|traceback|request[[:space:]]*(headers|body|dump)|response[[:space:]]*(headers|body|dump)|curl|https?://|<[^>]*>|<\?xml|xmlns[[:space:]]*=|eyJ[A-Za-z0-9_-]{8,}\.[A-Za-z0-9_-]{8,}\.[A-Za-z0-9_-]{8,}|[[:xdigit:]]{32,}|[A-Za-z0-9+/_-]{48,}={0,2}'
    );
$$;
  ...
  select jsonb_typeof(p_audit_metadata) = 'object'
    and not exists (
      ...
      where entry.key not in (
        ...
        or (entry.key = 'verification_upc' and (jsonb_typeof(entry.value) <> 'string' or entry.value #>> '{}' !~ '^[0-9]+$'))
...
begin
  ...
  elsif v_product_upc is null or v_product_upc !~ '^[0-9]+$' then
...
revoke all on function public.pos_publish_failure_message_is_safe(text) from public, anon, authenticated;
...
grant execute on function public.pos_publish_failure_message_is_safe(text) to service_role;

comment on function public.claim_pos_publish_job(uuid) is
  'Service-role-only atomic claim RPC. Returns safe update_price work only.';
comment on function public.report_pos_publish_job_status(uuid, uuid, text, text, numeric, text, text) is
  'Service-role-only guarded publishing status RPC. Never accepts connector credentials or raw Commander data.';
